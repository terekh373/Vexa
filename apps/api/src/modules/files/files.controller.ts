...
import { confirmUpload, createDownloadUrl, createUploadUrl, createVideoUploadUrl } from './files.service.js';
import { createUploadUrlSchema, createVideoUploadUrlSchema, fileIdParamsSchema } from './files.validation.js';
...
export const createVideoUploadUrlHandler: RequestHandler = async (req, res) => {
  const actor = requireActor(req.auth);
  const body = createVideoUploadUrlSchema.safeParse(req.body);
  ...
  const result = await createVideoUploadUrl(actor, body.data);
...
export const confirmUploadHandler: RequestHandler = async (req, res) => {
  ...
  // 202: Stream is still processing the video, the client should ask again.
  res.status(file.isReady ? 200 : 202).json({ file });
};
