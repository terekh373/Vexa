...
const Course = () => {
  ...
  const handleBuyCourse = () => {
    const newCourse = {
      ...
      module: 'Модуль 1. Вступ до 3D',
      progress: 0,
      image: 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=500&auto=format&fit=crop&q=60',
    };

    const savedCourses = JSON.parse(localStorage.getItem('my_courses') || '[]');
    const isAlreadyAdded = savedCourses.some((c) => c.title === newCourse.title);

    if (!isAlreadyAdded) {
      localStorage.setItem('my_courses', JSON.stringify([newCourse, ...savedCourses]));
    }

    alert('Курс успішно придбано та додано до Вашого простору!');
    navigate('/profile');
  };
        ...
        <section className={styles.aboutCourseBox}>
          <div className={styles.courseDetails}>
            ...
            <div className={styles.about}>
              ...
              <p>
                На цьому курсі ви з нуля опануєте основи роботи в Blender: розберетеся з інтерфейсом, навчитеся створювати та редагувати 3D-об'єкти, працювати з матеріалами, освітленням і камерою.
        ...
        <section className={styles.lessonsBox}>
          <div className={styles.modulesBox}>
            {lessonsModule.map((module, index) => (
              <LessonModule key={index} lesson={module} />
            ))}
          </div>
