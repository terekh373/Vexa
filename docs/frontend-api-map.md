...
- `quiz` — `null`, якщо в уроку немає тесту. Питання й варіанти віддаються
  ...
  перевірка відповідей залишається на сервері й буде додана в наступній
  частині issue #85.

**Урок безкоштовного курсу без запису поки що повертає `403`**: запис
`Enrollment` із `source = "FREE"` з'явиться в наступній частині #85, поки що
`priceAmount === 0` окремо не обробляється.
